    ...
    kvs::Light* light )
{
...
#if 0
    {
        ...
        kvs::Vec3 camera_position = camera->position();
        fprintf (stderr, "kvs::StochasticPolygonRenderer::setup(): camera=( % 13.6e, % 13.6e, % 13.6e )\n", camera_position.x(), camera_position.y(), camera_position.z());
        fprintf (stderr, "  + ModelViewMatrix:\n");
        fprintf (stderr, "   % 13.6e  % 13.6e  % 13.6e  % 13.6e\n", M[0][0], M[0][1], M[0][2], M[0][3]);
    ...
    kvs::Light* light )
{
    ...
    //std::cerr << "StochasticPolygonRendere::Engine::draw() : randomTexture ID=" << randomTex.id() << " (" << randomTex.width() << " x " << randomTex.height() << " x " << randomTex.depth() << ")" << std::endl;
#else
